...
// On a physical device replace with your machine's LAN IP, e.g. http://192.168.1.x:3000
const SERVER_URL = 'http://localhost:3000';

interface Game { id: number; time: string; }
interface Player { id: number; name: string; emailAddress: string; }
interface PlayerGame { id: number; gameId: number; playerId: number; score: number; Player: Player; }
...
export default function GamesScreen() {
  ...
  useEffect(() => { fetchGames(); fetchAllPlayers(); }, [fetchGames, fetchAllPlayers]);
  ...
  const removePlayer = (gameId: number, playerId: number) => {
    fetch(`${SERVER_URL}/Game/${gameId}/player/${playerId}`, { method: 'DELETE' })
      .then(() => setGamePlayers(prev => ({
        ...prev,
  ...
  const saveScore = (gameId: number, playerId: number) => {
    ...
    fetch(`${SERVER_URL}/Game/${gameId}/player/${playerId}`, {
      ...
      .then(() => setGamePlayers(prev => ({
        ...prev,
        [gameId]: prev[gameId].map(gp => gp.playerId === playerId ? { ...gp, score } : gp),
      })))
      .catch(console.error);
  };
  ...
  const playersNotInGame = (gameId: number) => {
    const inGame = new Set((gamePlayers[gameId] ?? []).map(gp => gp.playerId));
  ...
  return (
    <View style={s.container}>
      <TouchableOpacity style={s.primaryBtn} onPress={() => setShowNewGame(true)}>
        <Ionicons name="add-circle-outline" size={20} color="#25292e" />
          ...
          return (
            <View style={s.card}>
              <View style={s.cardRow}>
                <TouchableOpacity style={s.cardMain} onPress={() => toggleGame(game.id)}>
                  <Ionicons
                ...
                <TouchableOpacity onPress={() => deleteGame(game.id)} style={s.iconBtn}>
                  ...
                  {gpLoading[game.id] ? (
                    ...
                    <>
                      {players.map(gp => {
                        ...
                        return (
                          <View key={gp.playerId} style={s.playerRow}>
                            <Text style={s.playerName} numberOfLines={1}>{gp.Player?.name}</Text>
                            <TextInput
                              style={s.scoreInput}
                              value={scoreVal}
                              onChangeText={v => setEditScores(prev => ({ ...prev, [key]: v }))}
                              onSubmitEditing={() => saveScore(game.id, gp.playerId)}
                            ...
                            <TouchableOpacity
                              ...
                              style={s.iconBtn}
                            >
                              <Ionicons name="trash-outline" size={16} color="#e55" />
                            </TouchableOpacity>
                          </View>
                        );
                      })}
                      <TouchableOpacity
                        ...
                        onPress={() => { setAddPlayerGameId(game.id); setAddScore('0'); }}
                      >
                        <Ionicons name="person-add-outline" size={15} color="#ffd33d" />
      ...
      {/* ── New Game Modal ── */}
      <Modal visible={showNewGame} transparent animationType="fade" onRequestClose={() => setShowNewGame(false)}>
        <View style={s.overlay}>
          <View style={s.modal}>
            ...
            <View style={s.modalBtns}>
              <TouchableOpacity onPress={() => setShowNewGame(false)} style={s.cancelBtn}>
                <Text style={s.cancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={createGame} style={s.confirmBtn}>
        ...
        <View style={s.overlay}>
          <View style={s.modal}>
            ...
            <ScrollView style={{ maxHeight: 240 }}>
              {addPlayerGameId !== null && playersNotInGame(addPlayerGameId).length === 0 && (
                <Text style={s.emptyText}>All players are already in this game.</Text>
              )}
              {addPlayerGameId !== null && playersNotInGame(addPlayerGameId).map(p => (
                <TouchableOpacity
                  key={p.id}
            ...
            <TouchableOpacity onPress={() => setAddPlayerGameId(null)} style={[s.cancelBtn, { marginTop: 12, alignSelf: 'flex-end' }]}>
              <Text style={s.cancelText}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  scoreInput: {
    color: '#ffd33d', borderBottomWidth: 1, borderBottomColor: '#ffd33d',
    width: 64, textAlign: 'center', fontSize: 14, paddingVertical: 2, marginRight: 4,
  },
  ...
  playerOption: { paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#3a3f44' },
